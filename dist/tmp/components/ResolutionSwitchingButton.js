...
var VjsButtonResBBase = videojs.getComponent('Button');
...
var ResolutionSwitchingButton = (function (_VjsButtonResBBase) {
	_inherits(ResolutionSwitchingButton, _VjsButtonResBBase);

	function ResolutionSwitchingButton(player, options) {
		_classCallCheck(this, ResolutionSwitchingButton);

		_get(Object.getPrototypeOf(ResolutionSwitchingButton.prototype), 'constructor', this).call(this, player, options);
		this.init();
	}
  ...
  */
	...
	_createClass(ResolutionSwitchingButton, [{
		key: 'buildCSSClass',
		value: function buildCSSClass() {
			return 'vjs-ag-res-button ' + _get(Object.getPrototypeOf(ResolutionSwitchingButton.prototype), 'buildCSSClass', this).call(this);
		}
   ...
   */
		...
		value: function setCurrentResFromPlayer() {
   ...
   */
		...
		value: function setResolutionsNeededFromPlayer() {
			// Fallback
			this.resolutionsNeeded = false;
			// Real determination
			if (typeof this.typeAndTech == 'object') {
				var type = this.typeAndTech.type;
				if (this.sourcesByType[type] !== undefined && this.sourcesByType[type].length > 1) {
					for (var i = this.sourcesByType[type].length - 1; i >= 0; i--) {
						if (this.sourcesByType[type][i]['data-quality'] !== undefined || this.sourcesByType[type][i]['data-quality'] != this.currentRes) {
   ...
   */
		...
		value: function getAbsoluteSources() {
   ...
   */
		...
		value: function getSourceForResolutionChange() {
			var type = this.typeAndTech.type;
			var availableSources = this.sourcesByType[type];
			for (var i = availableSources.length - 1; i >= 0; i--) {
				if (this.currentRes == 'hd') {
					if (availableSources[i]['data-quality'] == undefined || availableSources[i]['data-quality'] !== 'hd') return availableSources[i];
				} else {
					if (availableSources[i]['data-quality'] == 'hd') return availableSources[i];
				}
			};
   ...
   */
		...
		value: function switchResolution() {
   ...
   */
		...
		value: function selectTypeAndTech(sources) {
			var techName;
			var tech;
   ...
   */
		...
		value: function selectResolution(typeSources) {
			var defaultRes = 0;
			...
			if (supportsLocalStorage) {
				...
				if (!isNaN(storedRes)) preferredRes = storedRes;
			}
  ...
  */
		...
		key: 'switchSource',
		value: function switchSource(new_source) {
   ...
   */
	...
	return ResolutionSwitchingButton;
})(VjsButtonResBBase);
