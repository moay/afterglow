...
class ResolutionSwitchingButton extends VjsButtonResBBase {
    ...
    this.sourcesByType = videojs.reduce(this.sources, (init, val, i) => {
  ...
  setResolutionsNeededFromPlayer() {
    ...
    if (typeof this.typeAndTech === 'object') {
      const type = this.typeAndTech.type;
      if (this.sourcesByType[type] !== undefined
				&& this.sourcesByType[type].length > 1) {
        for (let i = this.sourcesByType[type].length - 1; i >= 0; i--) {
  ...
  /**
	 ...
	 */
  removeSources() {
    	const videoEl = this.player_.el_.getElementsByTagName('video')[0];

    	if (this.player_.techName_ !== 'Html5' || !videoEl) return;

    	const srcs = videoEl.getElementsByTagName('source');
  ...
  getSourceForResolutionChange() {
    	const type = this.typeAndTech.type;
    	const availableSources = this.sourcesByType[type];
    	for (let i = availableSources.length - 1; i >= 0; i--) {
    		if (this.currentRes == 'hd') {
    			if (availableSources[i]['data-quality'] == undefined
    				|| availableSources[i]['data-quality'] !== 'hd') return availableSources[i];
    		} else if (availableSources[i]['data-quality'] == 'hd') return availableSources[i];
    	}
  ...
  /**
     ...
     */
 	switchResolution() {
 		const sourceToPlay = this.getSourceForResolutionChange();
  ...
  selectSource(sources) {
    ...
    const sourcesByType = this.sourcesByType;
    const typeAndTech = this.selectTypeAndTech(sources);
  ...
  selectTypeAndTech(sources) {
    	...
    	for (let i = 0, j = this.player_.options_.techOrder; i < j.length; i++) {
  ...
  selectResolution(typeSources) {
    ...
    // default
    let preferredRes = defaultRes;

    // trying to follow the videojs code conventions of if statements
    if (supportsLocalStorage) {
      const storedRes = parseInt(window.localStorage.getItem('videojs_preferred_res'), 10);

      if (!isNaN(storedRes)) preferredRes = storedRes;
    }

    const maxRes = (typeSources.length - 1);
    const actualRes = preferredRes > maxRes ? maxRes : preferredRes;
  ...
  switchSource(new_source) {
      ...
      return this.player_; // basically a no-op
	    ...
	    const remainPaused = this.player_.paused();
  ...
  /**
	 ...
	 */
  updateButton() {
    const buttonEl = this.prepareButtonElement(this.el_);
