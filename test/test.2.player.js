...
describe('Afterglow Player', () => {
  ...
  describe('setup', () => {
    ...
    it('should pass the skin name properly', () => {
      Player.prototype.getSkinName.restore();
      sinon.stub(Player.prototype, 'getSkinName').callsFake(() => 'testskinname');
      player = new Player(videoelement);
  ...
  describe('init() only inside of video.js ready()', () => {
    ...
    it('should set the volume properly if needed', () => {
      ...
      player.init();
      expect(player.videoelement.node.volumeInput).to.be.a('number');
  ...
  describe('prepareVideoElement regular', () => {
    ...
    beforeEach(() => {
      ...
      sinon.stub(Util.prototype, 'isYoutubePlayer').callsFake(() => false);
      sinon.stub(Player.prototype, 'applyVimeoClasses');
      sinon.stub(Util.prototype, 'isVimeoPlayer').callsFake(() => false);
      player = new Player();
  ...
  describe('prepareVideoElement youtube', () => {
    ...
    beforeEach(() => {
      ...
      sinon.stub(Player.prototype, 'applyParameters');
      sinon.stub(Player.prototype, 'applyYoutubeClasses');
      sinon.stub(Util.prototype, 'isYoutubePlayer').callsFake(() => true);
      sinon.stub(Player.prototype, 'applyVimeoClasses');
      sinon.stub(Util.prototype, 'isVimeoPlayer').callsFake(() => false);
      player = new Player();
      player.util = new Util();
  ...
  describe('prepareVideoElement vimeo', () => {
    ...
    beforeEach(() => {
      ...
      sinon.stub(Util.prototype, 'isYoutubePlayer').callsFake(() => false);
      sinon.stub(Player.prototype, 'applyVimeoClasses');
      sinon.stub(Util.prototype, 'isVimeoPlayer').callsFake(() => true);
      player = new Player();
      player.util = new Util();
    });
  ...
  describe('applyDefaultClasses noIE', () => {
    ...
    beforeEach(() => {
      sinon.stub(Player.prototype, 'setup');
      sinon.stub(Util.prototype, 'ie').callsFake(() => ({ isTheBrowser: 0, actualVersion: 0 }));
      sinon.stub(Config.prototype, 'getSkinClass').callsFake(() => 'testclass');
      sinon.stub(Config.prototype, 'init');
      player = new Player();
      player.util = new Util();
  ...
  describe('applyDefaultClasses IE', () => {
    ...
    beforeEach(() => {
      sinon.stub(Player.prototype, 'setup');
      sinon.stub(Util.prototype, 'ie').callsFake(() => ({ isTheBrowser: 0, actualVersion: 10 }));
      sinon.stub(Config.prototype, 'getSkinClass').callsFake(() => 'testclass');
      sinon.stub(Config.prototype, 'init');
      player = new Player();
      player.util = new Util();
  ...
  describe('applyParameters()', () => {
    ...
    beforeEach(() => {
      sinon.stub(Player.prototype, 'setup');
      sinon.stub(Player.prototype, 'calculateRatio').callsFake(() => 0.5);
      player = new Player();
    ...
    it('should set data-maxwidth if data-overscale is set', () => {
      ...
      sinon.stub(player.videoelement, 'getAttribute').callsFake(() => 'false');
    ...
    it('should not trigger responsivity stuff if has data-autoresize none', () => {
      ...
      player.videoelement.getAttribute.restore();
      sinon.stub(player.videoelement, 'getAttribute').callsFake(() => 'none');
      player.applyParameters();
    ...
    it('should get the ratio correctly and pass it to the correct places', () => {
      player.videoelement.getAttribute.restore();
      sinon.stub(player.videoelement, 'getAttribute').callsFake(() => 'fit');
      player.applyParameters();
  ...
  describe('applyYoutubeClasses()', () => {
    ...
    beforeEach(() => {
      sinon.stub(Player.prototype, 'setup');
      sinon.stub(Util.prototype, 'ie').callsFake(() => ({
        actualVersion: 0,
      }));
      sinon.stub(document, 'querySelector').callsFake(() => ({
        controls: false,
      }));
      ...
      navigator.__defineGetter__('platform', () => 'none', // customized user agent
      );
    ...
    it('should add class vjs-iOS for iPad devices', () => {
      navigator.__defineGetter__('platform', () => 'somthing120e7 19827e1982iPada8d7a928z8dhn', // customized user agent
      );
      player.applyYoutubeClasses();
      expect(player.videoelement.addClass).to.have.been.calledThrice;
    ...
    it('should add class vjs-iOS for iPod devices', () => {
      navigator.__defineGetter__('platform', () => 'somthing120e7 19827e1982iPoda8d7a928z8dhn', // customized user agent
      );
      player.applyYoutubeClasses();
      expect(player.videoelement.addClass).to.have.been.calledThrice;
      expect(player.videoelement.addClass).to.have.been.calledWith('vjs-iOS');
    ...
    it('should add class vjs-iOS for iPhone devices', () => {
      navigator.__defineGetter__('platform', () => 'somthing120e9827e1982iPhonea8d7a928z8dhn', // customized user agent
      );
      player.applyYoutubeClasses();
      expect(player.videoelement.addClass).to.have.been.calledThrice;
      expect(player.videoelement.addClass).to.have.been.calledWith('vjs-iOS');
  ...
  describe('calculateRatio()', () => {
    ...
    it('should return the correct ratio if it is already set', () => {
      player.videoelement.getAttribute.restore();
      sinon.stub(player.videoelement, 'getAttribute').callsFake(() => 0.243);
      const ratio = player.calculateRatio();
      expect(ratio).to.be.a('number');
    ...
    it('should return the correct ratio if widht and height are set', () => {
      player.videoelement.getAttribute.restore();
      sinon.stub(player.videoelement, 'getAttribute').callsFake((input) => {
        switch (input) {
          case 'height':
            return 1;
          case 'width':
  ...
  describe('destroy()', () => {
    ...
    it('should pause the player if it isnt already paused', () => {
      sinon.spy(player.videojs, 'pause');
      sinon.stub(player.videojs, 'paused').callsFake(() => false);
      player.destroy();
    ...
    it('should exit fullscreen if the player is fullscreen', () => {
      sinon.spy(player.videojs, 'exitFullscreen');
      sinon.stub(player.videojs, 'isFullscreen').callsFake(() => true);
      player.destroy();
  ...
  describe('getSkinName', () => {
    ...
    it('should return the data-skin parameter if it is set', () => {
      player.videoelement = {
        getAttribute: input => 'somevalue',
      };
      const res = player.getSkinName();
